...
def kft_Gen(data_Path,Index):
    ...
    if not os.path.exists(results_Path+"/KeyFrameTrajectory"+str(Index)+".txt"):
        ...
        os.system("./sfm ./colmap_ws "+data_Path+"/rgb")
        time2=time.time()
        time3=time2-time1
        os.system('./convertor')
        os.system('./sort')
...
def evo_data(data_Path,times):#输入某个数据集地址
    ...
    if not os.path.exists(results_Path+"/rgb.txt"):
        os.system("cp "+data_Path+"/rgb.txt "+results_Path+"/rgb.txt")
    if not os.path.exists(results_Path+"/groundtruth.txt"):
        os.system("cp "+data_Path+"/groundtruth.txt "+results_Path+"/groundtruth.txt")
    ...
    if not os.path.exists(results_Path+"/Results.txt"):
        ...
        print(data_results,end="",file=data_results)
...
#python evo_Tum.py ~/dataset/Tum/folders
