...
#文件准备,把rgb.txt和groundtruth.txt移动到根目录
    time3=0
    if not os.path.exists(results_Path+"/KeyFrameTrajectory"+str(Index)+".txt"):
        time1=time.time()
        if not os.path.exists("./colmap_ws"):
            os.system("mkdir colmap_ws")
        os.system("./sfm ./colmap_ws "+data_Path+"/mav0/cam0/data")
        time2=time.time()
        time3=time2-time1
        os.system('./convertor')
        os.system('./sort')
        ...
        os.system(cmd)
        if os.path.exists(results_Path+"/KeyFrameTrajectory"+str(Index)+".txt") :
            os.system("rm -rf ./colmap_ws/*")
    return time3
...
def evo_data(data_Path,times):#输入某个数据集地址
    data_Name = os.path.basename(data_Path)
    results_Path="./Results/"+data_Name
    if not os.path.exists("./sfm") or \
       not os.path.exists("./convertor") or \
       not os.path.exists("./sort") or \
       not os.path.exists("./modify_Precision") or \
    ...
    if not os.path.exists(results_Path+"/groundtruth.csv"):
        cmd="cp "+data_Path+"/mav0/state_groundtruth_estimate0/data.csv "\
            +results_Path+"/groundtruth.csv"
        os.system(cmd)
    ...
    for i in range(0,times):
        ...
        cmd="./tracking_Rate "+results_Path+"/rgb.csv "\
            +results_Path+"/KeyFrameTrajectory"+str(i+1)+".txt "+results_Path+"/Results.txt"
