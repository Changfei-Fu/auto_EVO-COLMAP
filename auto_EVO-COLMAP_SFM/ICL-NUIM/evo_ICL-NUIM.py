...
def kft_Gen(data_Path,Index):
    ...
    if not os.path.exists(results_Path+'/KeyFrameTrajectory'+str(Index)+'.txt'): 
        rgb_txt=open(results_Path+"/rgb.txt")
        image_num=len(rgb_txt.readlines())
        time1=time.time()
        ...
        os.system('./convertor')
        os.system('./sort')
        os.system("./prepare "+str(image_num)+" "+results_Path+"/rgb.txt ./KeyFrameTrajectory.txt")
        cmd="mv KeyFrameTrajectory.txt "+results_Path+"/KeyFrameTrajectory"+str(Index)+".txt"
        os.system(cmd)
        os.system('mv ./rgb.txt '+results_Path+"/rgb.txt")
...
def evo_data(data_Path,times):#输入某个数据集地址
    data_Name = os.path.basename(data_Path)
    results_Path="./Results/"+data_Name
    if not os.path.exists("./convertor") or \
       not os.path.exists("./prepare") or \
       not os.path.exists("./sfm") or \
